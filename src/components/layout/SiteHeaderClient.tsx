...
export function SiteHeaderClient({
  ...
  const phoneLabel = phoneDisplay ?? "Контакты";
  const phoneLink = phoneHref ?? "/contacts";
    ...
    <header className="relative border-b border-slate-100 bg-white">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3">
        ...
        <nav className="hidden items-center gap-6 text-sm text-slate-600 md:flex">
          <Link href="/" className="transition-colors hover:text-[#1D2D44]">
            Главная
          </Link>
          <Link
            href="/services"
        ...
        <div className="hidden items-center gap-4 md:flex">
          <a href={phoneLink} className="text-sm font-medium text-[#0D1321]">
            {phoneLabel}
          </a>
          <a
            href="/contacts"
